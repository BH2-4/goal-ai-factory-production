...
三位提交人各自独立报名、独立提交，作品互不引用。每人的完整材料包：
...
**给三位开发者的最短路径**：`最简PRD_*.md`（开工图纸，七段结构）→ `后续思路补充说明.md`（背景与边界，对内）。

内部材料（决策日志/评分/审计/审核 deck）留在 `docs/` 与 `slides/`，**不进入任何提交物**；三作复赛代码须建独立仓库、互不共享源码（`docs/research/三作品独立性审计.md`）。
