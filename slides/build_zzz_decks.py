...
DECKS = {
 ...
 'M2C': dict(acc='#1eaeb7', deep='#137378', code='ZZ-03', brand='PAPER → PROFILE KIOSK',
   name='气脉建档 AirDoc', sub='设备档案自动建立 · 涂鸦扫描亭 · 非官方审核稿',
   h1='非标单据，<span class="accent">不该靠人手抄进系统</span>',
   lede='文档理解、校验对齐、发布三段流水：解析、预填、冲突高亮全自动；人工确认后入库，设备自动进入保养与监测。',
   marks=['字段可溯源','置信度分级','人审定稿']),
...
META = {
 ...
 'M2C': dict(score='83.0 分', tier='模块库', anchor='S07 装机验收档案数字化'),
...
'M2C': [
 dict(f='cover'),
 dict(f='anchor3', rows=[
   dict(name='S07 装机验收档案数字化', score='83.4 ✓', desc='与售后主链、机队哨兵构成「装机→运维→哨兵」同行业闭环'),
   dict(name='S09 二手设备评估档案', score='80.1', desc='偏交易评估域，叙事游离主线'),
   dict(name='S08 移机档案迁移', score='78.9', desc='频次低，痛点中低频')],
   pick='和主线锁在一起的一个', reason='单看分数它不占优，但它是 P2 数据域的第一块砖：每台新机的档案，就是售后主链与机队哨兵的输入。'),
 dict(f='pain', big='30', unit=' 分钟', h2='一份装机单的录入成本', flow=[
   '手写、拍照、PDF 三种格式混排','新机滞后入监测，保养提醒漏发','保内保外判定纠纷'],
   note='自造 20-30 份多格式样例（版权干净）：手写装机单、Excel 验收表、PDF 铭牌照片——脏乱差本身就是题目。'),
 dict(f='formula', parts=['多格式单据','+','主数据校验','→','结构化档案 + 保养基线'],
   lede='每个字段带来源坐标与置信度；人工确认不是补丁，是产品流程本身。'),
 dict(f='agents', cards=[
   dict(n='文档理解 Agent', d='版面分析 + 表格字段抽取 + 手写 OCR，输出带来源坐标与置信度'),
   dict(n='校验对齐 Agent', d='匹配机型/序列号主数据，单位与料号规范校验，冲突高亮'),
   dict(n='发布 Agent', d='待确认清单 →（人审后）写入档案库 + 自动注册保养基线与监测订阅')],
   loop_line='微循环：上传 → 解析 → 预填 + 冲突高亮，全程自主',
   gate_line='宏门禁：档案定稿 = 工程师确认；写入下游前留痕审核'),
 dict(f='steps7', names=S7, notes='七步落点：多格式文档输入、类型与字段集识别、解析校验计划、OCR/版面/主数据调用、结构化档案+置信度+冲突清单、字段级准确率统计、数据授权与留痕。'),
 dict(f='evidence', items=[
   ('≥95%','打印件字段级准确率（手写件 ≥85%）'),
   ('≤60s','单份处理时长'),
   ('100%','档案注册后保养提醒触达率')],
   lede='字段级 ground truth 人工标注——修正量 ≤10% 的门禁，让"识别不稳"变成"置信度分级"的产品特性。'),
 dict(f='boundary', title='扫描亭不越权入库', rows=[
   ('低风险建议域', '定稿与写入均需人工确认，全程留痕可审计（手册 8.1-5/8.1-6）'),
   ('样例不含真实客户信息', '自造多格式样例，版权干净（FAQ Q7）')],
   lede='它解决的是"售后 AI 的第一公里"——数据脏乱差挡住 AI 提效，这里就是发生地。'),
 dict(f='timeline3', rows=[('初赛 8.16','P2 叙事「数据底座」页','复赛 9.3','第二演示链（时间富余时）'),('决赛 9.22','现场上传新样例即时解析','角色','决赛扩展位')]),
 dict(f='opensource', rows=[
   dict(n='装机单 schema', d='字段级结构定义'),
   dict(n='抽取模板', d='Prompt + 后处理管线'),
   dict(n='多格式样例集', d='20-30 份带 ground truth')],
   risk='手写识别不稳 → 置信度分级 + 人工修正闭环即卖点；场景价值分低 → 绑定 P2 主线叙事。'),
 dict(f='intro8', paras=[
   ('项目名称：','气脉建档 AirDoc——装备装机验收档案数字化智能体'),('行业赛题：','AI+工业制造'),
   ('目标用户：','装备厂商客服、档案管理员与售后工程师'),('核心问题：','装机验收单手写、拍照、PDF 混排，档案录入滞后易错，设备未及时纳入保养与监测'),
   ('解决方案：','三 Agent 流水线：文档理解做版面分析与字段抽取（带来源坐标与置信度），校验对齐匹配主数据并高亮冲突，发布在人工审核后写入档案库、自动注册保养基线'),
   ('创新点：','每个字段可溯源（来源坐标+置信度），人工确认内建为产品环节；正面解决"数据脏乱差挡住 AI 提效"的行业堵点'),
   ('开放/复用价值：','开源装机单 schema、抽取模板与多格式样例集，可迁移至其他装备与工程文档场景'),('当前进展：','样例集制作与原型开发中，提交时附解析演示')]),
 dict(f='review', asks=['涂鸦扫描亭的气质是否成立？','字段溯源的表达够不够直观？','作为决赛扩展位，独立成 deck 是否必要？']),
],
'M3B': [
 dict(f='cover'),
 dict(f='anchor3', rows=[
